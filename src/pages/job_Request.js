...
const HRJobDashboard = () => {
  ...
  const handleStatusChange = async (appId, applicantName, newStatus, email) => {
    const currentSelection = selections[appId];
    const message =
      messages[appId] ?? currentSelection?.message_to_user ?? "";
    ...
    try {
      ...
      if (currentSelection) {
        // UPDATE using email
        res = await axios.put(
          `${API_URL}/selections/selection/by-email?email=${email}`,
          {
            selection_status: newStatus,
            message_to_user: message,
          }
        ...
        // CREATE if no existing selection
        res = await axios.post(`${API_URL}/selections`, {
          job_applicant_id: appId,
          applicant_name: applicantName,
          selection_status: newStatus,
